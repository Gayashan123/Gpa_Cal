  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row">
        {/* Sidebar */}
        <div className={`w-full md:w-1/3 p-8 ${ACCENT_COLOR}`} style={{ backgroundColor: PRIMARY_COLOR }}>
          {/* Avatar */}
            ...
            <div className="relative">
              {personalDetails.photoPreview ? (
                ...
                <div className="w-32 h-32 rounded-full bg-white bg-opacity-20 border-4 border-white shadow-lg flex items-center justify-center">
                  <span className="text-5xl font-bold text-white">{nameInitial}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-bold border-b border-white border-opacity-30 pb-2 mb-4">CONTACT</h3>
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-sm opacity-90">{personalDetails.contactNumber}</p>
              </div>
            </div>
            <div className="flex items-start">
              <div className="mr-3 mt-1">
              ...
              <div>
                ...
                <p className="text-sm opacity-90 break-all">{personalDetails.email}</p>
              </div>
            </div>
            {personalDetails.address && (
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm opacity-90">{personalDetails.address}</p>
                </div>
              </div>
            )}
                ...
                <div>
                  ...
                  <p className="text-sm opacity-90">
                    ...
                    {personalDetails.dateOfBirth && (
                      <span className={personalDetails.nic ? "ml-3" : ""}>
          ...
          {skills && (
            <div className="mt-8">
              <h3 className="text-xl font-bold border-b border-white border-opacity-30 pb-2 mb-4">SKILLS</h3>
          ...
          {languages.length > 0 && (
            <div className="mt-8">
              <h3 className="text-xl font-bold border-b border-white border-opacity-30 pb-2 mb-4">LANGUAGES</h3>
          ...
          {interests && (
            <div className="mt-8">
              <h3 className="text-xl font-bold border-b border-white border-opacity-30 pb-2 mb-4">INTERESTS</h3>
              <p className="text-sm opacity-90">{interests}</p>
            </div>
          )}
        </div>
        ...
        <div className="w-full md:w-2/3 p-8">
          {/* Name and Title */}
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-gray-800 mb-1">{personalDetails.fullName}</h1>
          </div>
          ...
          {professionalExperience.length > 0 && (
            <section className="mb-8">
              <div className="flex items-center mb-4">
                <div className="w-8 h-1 bg-blue-900 mr-3" style={{ backgroundColor: PRIMARY_COLOR }}></div>
              ...
              <div className="space-y-6">
                {professionalExperience.map((exp, i) => (
                  <div key={i} className="pl-4 border-l-2 border-gray-200 relative">
                    <div className="absolute -left-2.5 top-1 w-4 h-4 rounded-full border-2 border-white" style={{ backgroundColor: PRIMARY_COLOR }}></div>
                    <div className="ml-2">
                      ...
                      <div className="text-gray-700 font-medium mb-2">{exp.organization}</div>
                      {exp.responsibilities && (
                        <ul className="mt-2 text-gray-700 space-y-1 list-disc list-inside">
                          {exp.responsibilities.split('\n').map((item, idx) => (
                            item.trim() && <li key={idx} className="text-sm">{item}</li>
                          ))}
                        </ul>
                      )}
                    </div>
              ...
              <div className="space-y-4">
                {education.map((edu, i) => (
                  <div key={i} className="pl-4 border-l-2 border-gray-200 relative">
                    ...
                    <div className="ml-2">
                      <div className="flex justify-between items-baseline">
                        <h3 className="text-lg font-bold text-gray-800">{edu.qualification}</h3>
                        <span className="text-sm text-gray-600 bg-gray-100 px-2 py-1 rounded">{edu.year}</span>
                      </div>
            ...
            <section className="mb-8">
              ...
              {olResults.indexNo && (
                <div className="mb-6">
                  ...
                  <div className="mb-2 text-sm text-gray-600">Index No: {olResults.indexNo}</div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-left border-b-2 border-gray-200">
                          <th className="p-2 font-semibold">Subject</th>
                          <th className="p-2 font-semibold text-right">Grade</th>
                        </tr>
                      </thead>
                      <tbody>
                        {olResults.subjects.map((subj, i) => (
                          <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                            ...
                            <td className="p-2 text-right font-medium">{subj.grade}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {otherQualifications.map((qual, i) => (
                  <div key={i} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="flex justify-between items-baseline">
                      <h3 className="font-bold text-gray-800">{qual.title}</h3>
                      <span className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded">{qual.year}</span>
                    </div>
                    <div className="text-gray-700 text-sm mt-1">{qual.institution}</div>
                  </div>
                ))}
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {references.map((ref, i) => (
                  <div key={i} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="text-gray-700 text-sm">{ref.position}, {ref.organization}</div>
